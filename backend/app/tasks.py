...
from .redis_utils import get_redis, publish_message, cache_result
from .analytics import track_processing_started, track_processing_completed, track_processing_failed
...
def process_engineering_drawing_task(
    ...
    try:
        ...
        # Track processing started event
        track_processing_started(batch_id=batch_id, image_size=image.size)
        ...
        result = {
            "detected_symbols": detected_texts,
        ...
        # Cache the full result with image in Redis (1 hour TTL)
        cache_result(f"batch_result:{batch_id}", result, ttl=3600)
        
        # Track processing completed event
        processing_time = time.time() - start_time
        high_confidence_count = sum(1 for s in detected_texts if s.get('confidence', 0) > 0.8)
        ...
        logger.info(f"OCR processing completed for batch {batch_id}. Found {len(detected_texts)} text regions.")
        return result
            
    except Exception as e:
        error_msg = f"Error processing engineering drawing: {str(e)}"
        logger.error(error_msg, exc_info=True)
        
        # Track processing failed event
