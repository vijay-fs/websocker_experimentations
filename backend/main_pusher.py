...
async def send_to_pusher(batch_id: str, message: str, progress: int, result: Optional[Dict[str, Any]] = None, message_type: str = "info"):
    ...
    try:
        ...
        # Use the official Pusher library - much simpler!
        response = pusher_client.trigger(channel_name, event_name, data)

        logger.info(f"📡 Pusher response: {response}")

        # For the official library, an empty response {} often means success
        if response is None or response == {} or response.get('status_code') in [200, 201] or response.get('status') in [200, 201]:
...
async def upload_file(file: UploadFile = File(...)):
    """File upload that sends real-time progress updates via Pusher"""
    ...
    try:
        ...
        await send_to_pusher(batch_id, "File uploaded successfully!", 25)

        # Simulate processing stages
        await asyncio.sleep(1)
        await send_to_pusher(batch_id, "Analyzing file content...", 50)

        await asyncio.sleep(1)
        await send_to_pusher(batch_id, "Processing image data...", 75)
        ...
        result = {
            ...
            "processing_time": "3 seconds (simulated)"
        }
